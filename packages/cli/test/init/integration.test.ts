...
function makeTmpDirs(): { synapseHome: string; brainDir: string; cleanup: () => void } {
    const synapseHome = mkdtempSync(join(tmpdir(), 'synapse-int-home-'));
    const brainDir = mkdtempSync(join(tmpdir(), 'synapse-int-brain-'));
    return {
        synapseHome,
        brainDir,
        cleanup: () => {
...
describe('SmartInit integration', () => {
    it('produces complete overview with all sections', () => {
        ...
        try {
            ...
            expect(overview).toContain('Auto-generated by Synapse');
        } finally {
            cleanup();
        }
    });
    ...
    it('creates DNA entries', () => {
        ...
        try {
            ...
            const dnaDir = join(synapseHome, 'dna');
