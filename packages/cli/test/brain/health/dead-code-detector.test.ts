...
describe('DeadCodeDetector', () => {
    ...
    it('returns empty array when all functions are referenced', () => {
        repo.insertNodes([
        ...
        const dead = detector.detect();
        expect(dead).toEqual([]);
    });

    it('detects functions with zero dependents', () => {
        repo.insertNodes([
            { id: 'fn:a.ts:main', type: 'function', name: 'main', filePath: 'a.ts', lineStart: 1, lineEnd: 5 },
            { id: 'fn:b.ts:helper', type: 'function', name: 'helper', filePath: 'b.ts', lineStart: 1, lineEnd: 3 },
    ...
    it('excludes entry point files from dead code', () => {
        repo.insertNodes([
    ...
    it('detects unreferenced classes', () => {
        repo.insertNodes([
    ...
    it('includes reason for each dead code entry', () => {
        repo.insertNodes([
