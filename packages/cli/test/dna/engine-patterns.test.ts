...
describe('DnaEngine - Pattern Matching', () => {
    ...
    it('finds semantically similar instructions', async () => {
        ...
        expect(matches[0].similarity).toBeGreaterThan(
            SIMILARITY_THRESHOLD_FOR_TEST
        );
    }, 60000);
    ...
    it('merges similar instructions when capturing with embeddings', async () => {
        ...
        const entry =
            await engine.captureInstructionWithPatternMatch(
                'Always return early from functions instead of nesting',
                'session-2',
                'correction'
            );
        ...
        const styleEntries = allEntries.filter(
            (e) => e.frontmatter.category === 'style'
        );
    ...
    it('does not merge instructions from different categories', async () => {
        ...
        const antiPatternEntries = storage
            .listEntries()
            .filter(
                (e) => e.frontmatter.category === 'anti-patterns'
            );
        const preferenceEntries = storage
            .listEntries()
            .filter(
                (e) => e.frontmatter.category === 'preferences'
            );

        expect(
            antiPatternEntries.length + preferenceEntries.length
